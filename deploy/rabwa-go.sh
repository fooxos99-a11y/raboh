...
set -euo pipefail
trap 'echo "=== توقف عند السطر $LINENO: $BASH_COMMAND"' ERR
ROOT=/var/www/rboh
...
echo "=== تثبيت المكتبات"
if [ -d node_modules/mysql2 ] && [ -d node_modules/express ]; then
  echo "المكتبات مثبتة من قبل"
else
  if ! npm ci --omit=dev --ignore-scripts --no-audit --no-fund >/tmp/rabwa-npm.log 2>&1; then
    echo "فشل تثبيت المكتبات:"; tail -30 /tmp/rabwa-npm.log; exit 1
  fi
  echo "تم التثبيت"
fi
