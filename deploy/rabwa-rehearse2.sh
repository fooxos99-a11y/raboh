...
echo "=== تثبيت المكتبات"
[ -d node_modules/mysql2 ] && echo "المكتبات مثبتة من قبل" || npm ci --omit=dev --ignore-scripts --no-audit --no-fund >/tmp/rabwa-npm.log 2>&1 || { tail -20 /tmp/rabwa-npm.log; exit 1; }
